    ...
    public class TarodevAutoSave : Editor {
        ...
        private static async Task SaveInterval(CancellationToken token) {
            while (!token.IsCancellationRequested) {
                ...
                if (_config.Logging) Debug.Log($"[<color=green><b>Surge Breaker Crisis-Prevention Agent</b></color>] Auto-saved <color=#03fc7f>Scene {(SceneManager.GetActiveScene().name)}</color> at {DateTime.Now:h:mm:ss tt}..");
            }
        }
