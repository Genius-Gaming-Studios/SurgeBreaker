    ...
    public class TarodevAutoSave : Editor {
        ...
        private static async Task SaveInterval(CancellationToken token) {
            while (!token.IsCancellationRequested) {
                ...
                if (_config.Logging) Debug.Log($"<color=#1e6b00><b>[Surge Breaker Crisis Prevention Agent]</b></color> Auto-saved Scene <color=#47ff00>{SceneManager.GetActiveScene().name}</color> at {DateTime.Now:h:mm:ss tt}");
            }
        }
